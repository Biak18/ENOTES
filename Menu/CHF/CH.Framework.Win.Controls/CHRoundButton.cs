...
public partial class CHRoundButton : Button
{
    ...
    private GraphicsPath GetFigurePath(Rectangle rect, float radius)
    {
        ...
        path.StartFigure();
        path.AddArc(rect.X, rect.Y, radius, radius, 180, 90); // TL
        //path.AddArc(rect.Width - radius, rect.Y, radius, radius, 270, 90);
        //path.AddArc(rect.Width - radius, rect.Height - radius, radius, radius, 0, 90);
        //path.AddArc(rect.X, rect.Height - radius, radius, radius, 90, 90);

        path.AddArc(rect.Right - radius, rect.Y, radius, radius, 270, 90); // TR
        path.AddArc(rect.Right - radius, rect.Bottom - radius, radius, radius, 0, 90);  // BR
        path.AddArc(rect.X, rect.Bottom - radius, radius, radius, 90, 90); // BL
        path.CloseFigure();
        return path;
    }
